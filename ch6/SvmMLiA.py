...
import random
import numpy as np
from ch1.kNN import img2vector
...
def smoSimple(dataMatIn, classLabels, C, toler, maxIter) :
    ...
    b = 0; shape = np.shape(dataMatrix); m = shape[0]
        ...
        for i in range(m) :
            ...
            if ((labelMat[i] * Ei < -toler) and (alphas[i] < C)) or ((labelMat[i] * Ei > toler) and (alphas[i] > 0)) :
                ...
                if (0 < alphas[i]) and (C > alphas[i]) :
                    ...
                    b = (b1 + b2) / 2.0
                alphaPairsChanged += 1
                print("iter: %d i:%d, pairs changed %d" % (index, i, alphaPairsChanged))
        if (alphaPairsChanged == 0) :
            index += 1
        else :
            index = 0
        print("iteration number: %d" % index)
    return b, alphas
...
def testDigits(kTup = ('rbf', 10)) :
    ...
    errorCount = 0
    for i in range(m) :
        kernelEval = kernelTrans(sVs, dataMat[i, :], ('rbf', kTup))
        predict = kernelEval.T * np.multiply(labelSV, alphas[svInd]) + b
        if np.sign(predict) != np.sign(labelArr[i]) :
            errorCount += 1
    print("the training error rate is: %f" % (float(errorCount) / m))
